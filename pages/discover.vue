<template>
	<div class="discover-page">
        ...
        <div class="root-container">
            ...
            >
                <GmapMarker 
                    v-for="(shift, i) in shiftsByState" :key="i" 
...
export default {
    ...
    computed: {
        ...
        shiftsByCity() {
            return this.shiftsByState.filter(s => s.city === this.userData.city.toLowerCase());
        },
        shiftsForList() {
            return this.viewType === "city" ? this.shiftsByCity : this.shiftsByState;
        }
    },
    async created() {
        this.cityCoords = { lat: this.userData.lat, lng: this.userData.lng };
        try {
            const shiftReq = await this.$axios(`${process.env.BASE_URL}/get-shifts-by-state`, {
                method: "POST",
                headers: {
